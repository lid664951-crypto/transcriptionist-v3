...
## 🆕 v1.1.0 重大更新（2026-01-28）

> 本版本聚焦「库界面重构」「大体量音效库的性能稳定性」以及「AI 检索/打标的深层修复」，目标是让**数万条音效库依然可用且流畅**。

你可以把这次更新理解为：**界面结构重构 + 性能底座重做 + AI 质量与稳定性全面修复**。

### ⭐ 版本亮点（Highlights）

- **库板块重构：文件夹树与音效列表分离**
  - `LibraryPage` 变为“纯文件夹树”，不再在树上挂载海量文件节点，避免大库卡死。
  - 新增/强化独立的 **音效列表面板 `AudioFilesPanel`**：专门负责展示文件、筛选、选择、播放与右键操作。

- **音效列表升级为“虚拟列表”**
  - 从 `QTreeWidgetItem` 切换为 `QTreeView + QAbstractTableModel`，只渲染可见行。
  - 大幅降低内存占用与 UI 卡顿，滚动/选择在千级以上条目仍保持流畅。

- **大库性能与稳定性增强（数万条可用）**
  - 构建“文件夹 → 文件索引列表”映射，勾选文件夹不再做全库 O(N×M) 扫描。
  - 搜索加入 **防抖（debounce）**，避免快速输入触发频繁全库搜索导致卡顿。
  - 导入/保存等操作引入批处理策略，降低 SQLite 变量数量与事务压力，提升稳定性。

- **AI 检索/打标质量关键修复**
  - 修复 CLAP embedding **未做 L2 归一化**导致相似度不准的问题，检索精准度显著提升。
  - 智能打标针对不同标签集优化阈值（尤其是长句子标签集），并限制最多返回前 10 个标签，兼顾质量与可读性。

- **标签页与打标流程稳定性修复**
  - 修复标签页懒加载在某些布局下只显示首批（例如 20 个）的情况，并增强加载状态提示。
  - 修复打标过程中“索引路径与数据库路径分隔符不一致（`/` vs `\\`）”导致成功数为 0 的问题，并增加诊断日志。

- **设置持久化修复**
  - 修复“AI 批量翻译性能（批次/并发/网络档位）重启恢复默认”的覆盖写回问题，现可稳定保存到 `config/config.json`。

### 🧩 详细变更（按模块）

- **库与浏览体验**
  - 文件夹树专注层级结构，音效列表专注文件展示与操作（播放/定位/从库移除等）。
  - 全选仍支持把“全库文件集合”交给 AI 模块处理，但 UI 不会尝试一次性渲染全库列表，避免卡死。

- **性能优化**
  - 大批量导入（如 6000+）并行策略优化：独立子进程阈值下调至更合理范围，CPU 利用率显著提高。
  - 搜索防抖与批处理写入，减少无意义的重复计算与数据库压力。

- **数据库/线程安全与重命名**
  - Worker 线程使用独立 Session（`session_scope()`），避免跨线程复用 Session 的潜在问题。
  - 重命名增加写权限预检查、异常日志与更友好提示；路径更新逻辑更安全，避免误匹配。

### 📎 相关文档（建议一起发布到 GitHub）

- **完整更新日志**：`CHANGELOG_v1.0.1.md`（包含 v1.1.0 Major 详细说明）
- **AI 修复报告**：`BUG_FIXES_REPORT.md`、`AI_BUGS_FIXED_SUMMARY.md`
- **导入性能优化说明**：`IMPORT_OPTIMIZATION.md`
- **快速验证导入优化**：`QUICK_TEST_IMPORT.md`
...
**GPU 加速批量大小**（当前 CLAP 为 larger-clap-general，显存占用较旧版高）：
- 显存 2–4GB：建议 2
- 显存 4–8GB：建议 4
- 显存 8GB+：建议 4 或 8（显存不足时调小）

**CPU 音频预处理并行数**（用于建立索引时的音频预处理，与 CLAP 模型无关）：
- 软件按本机 CPU 自动推荐（保守上限 6 进程），也可自定义 1–8。

软件会根据你的硬件自动推荐，也可在设置中自行调整。
